...
void check_tree(int *array, size_t size_init, size_t size, size_t i)
{
	...
	branch1 = array[br1];
	branch2 = array[br2];
	if (((br1 < size) && (br2 < size) && (branch1 >= branch2 && branch1 > array[i])) || ((br1 == size - 1) && branch1 > array[i]))
	{
		n = array[i];
		array[i] = branch1;
		array[br1] = n;
	...
	else if ((br1 < size) && (br2 < size) && (branch2 > branch1 && branch2 > arry[i]))
	{
		n = array[i];
		array[i] = branch2;
		array[br2] = n;
...
/**
 * heap_sort - sorts an array og integers
...
{
	size_t i, size_init, size_t size;
	int n;
	...
	for (i = 0; i < size / 2 ; i++)
	{
		check_tree(array, size_init, size / 2 - 1 - i);
	}
	for (i = 0; i < size_init - 1 - i; i++)
	{
		n = array[0];
